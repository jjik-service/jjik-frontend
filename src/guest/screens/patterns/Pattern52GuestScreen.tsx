// ✅ 파일 위치: src/screens/guest/screens/Pattern52GuestScreen.tsx
// 52번 패턴: 사용자 번호 + 주소/주민번호 기반 - 게스트용
// 고정번호/제외번호 입력창 4개씩 + 주소/주민번호 입력창 + 백엔드 API 연동
...
export default function Pattern52GuestScreen() {
  ...
  const [address, setAddress] = useState<string>(""); // 주소 입력
  const [ssnFirst, setSsnFirst] = useState<string>(""); // 주민번호 앞자리
  const [ssnLast, setSsnLast] = useState<string>(""); // 주민번호 뒷자리
  ...
  return (
    <View style={styles.container}>
      {/* ✅ 배경 이미지 
      <Image 
        source={require('../../../../assets/images/guest/patterns/pattern52_guest.png')} 
      ...
      {/* ✅ 주민번호 앞자리 입력창 */}
      <TextInput
        style={styles.ssnFirstInput}
        value={ssnFirst}
        onChangeText={setSsnFirst}
        placeholder="123456"
        keyboardType="numeric"
        maxLength={6}
        multiline={false}
      ...
      {/* ✅ 주민번호 뒷자리 입력창 */}
      <TextInput
        style={styles.ssnLastInput}
        value={ssnLast}
        onChangeText={setSsnLast}
        placeholder="1234567"
        keyboardType="numeric"
        maxLength={7}
        multiline={false}
...
const styles = StyleSheet.create({
  ...
  // ✅ 공통 입력창 스타일 (51번과 완전히 동일, 노란색 외곽선)
  numberInput: {
    position: 'absolute',
  ...
  addressInput: {
    position: 'absolute',
    top: scaleHeight(1285),
    left: scaleWidth(55),
    width: scaleWidth(990),
    height: scaleHeight(100),
    backgroundColor: '#FFFFFF',
    fontSize: scaleFontSize(54),
    color: '#000000',
    textAlign: 'center',
    textAlignVertical: 'bottom',
    paddingBottom: scaleHeight(10),
