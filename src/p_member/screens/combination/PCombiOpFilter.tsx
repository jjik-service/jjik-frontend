// ✅ 파일 위치: src/p_member/screens/combination/PCombiOptionFilter.tsx
// ✅ 배경 이미지: assets/images/p_member/combination/p_my_combination_option_filter.png
// ✅ 프리미엄 옵션형 패턴 필터링 조합 화면
...
export default function PCombiOptionFilter() {
  ...
  const handleGenerationConfirm = useCallback(async () => {
    ...
    try {
      // ✅ 백엔드 API 호출 - 옵션형 패턴 필터링 조합
      const response = await fetch('/api/option-patterns/filter', {
        method: 'POST',
        headers: {
              ...
              <View style={styles.checkboxContainer}>
                ...
                <TouchableOpacity
                  style={[styles.calendarButton, {
                    width: (width * 0.9) / 1000 * 110,
                    height: (width * 0.9) / 1000 * 110,
                    marginLeft: (width * 0.9) / 1000 * 880, // 882 - 360 - 110 = 412
                  }]}
        ...
        <View style={styles.optionButtonsContainer}>
          {optionInfo.options && optionInfo.options.map((option, index) => {
            ...
            if (currentOptionPattern === 1 || currentOptionPattern === 2 || currentOptionPattern === 3) {
              ...
              buttonTop = buttonsStartY + 100; // 고수/저수 레이블 아래
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* ✅ 패턴 선택 1 드롭다운 (x65, y455, w655, h110) */}
      <TouchableOpacity
      ...
      {/* ✅ 패턴 선택 2 드롭다운 (x65, y635, w655, h110) */}
      <TouchableOpacity
        style={[styles.dropdownButton, {
      ...
      {/* ✅ 패턴 선택 3 드롭다운 (x65, y820, w655, h110) */}
      <TouchableOpacity
        style={[styles.dropdownButton, {
          top: (815 / DESIGN_HEIGHT) * height,
          left: (63 / DESIGN_WIDTH) * width,
          width: (655 / DESIGN_WIDTH) * width,
          height: (110 / DESIGN_HEIGHT) * height,
        }]}
      ...
      {/* ✅ 패턴 3 제외 체크박스 (x787, y835, w80, h80) */}
      <TouchableOpacity
        style={[styles.button, {
      ...
      {/* ✅ 필터링 조합 번호 생성 버튼 (x62, y1325, w968, h164) */}
      <TouchableOpacity
