// ✅ 파일 위치: src/p_member/screens/combination/PCombiGenCompose.tsx
// ✅ 배경 이미지: assets/images/p_member/combination/p_my_combination_general_compose.png
// ✅ 프리미엄 일반 패턴 합성 조합 화면
...
export default function PCombiGenCompose() {
  ...
  const handleGenerationConfirm = useCallback(async () => {
    ...
    try {
      // ✅ 백엔드 API 호출 - 일반 패턴 합성 조합
      const response = await fetch('/api/general-patterns/compose', {
        method: 'POST',
        headers: {
...
/>
      ...
      {/* ✅ 합성 조합 번호 생성 버튼 (x62, y1325, w968, h164) - 옵션형과 동일 */}
      <TouchableOpacity
      ...
      {/* ✅ 나만의 조합 저장하기 버튼 (x128, y1749, w650, h100) */}
      <TouchableOpacity
        style={[styles.button, {
          top: (1749 / DESIGN_HEIGHT) * height,
          left: (128 / DESIGN_WIDTH) * width,
      ...
      <TouchableOpacity
        style={[styles.button, {
          top: (1749 / DESIGN_HEIGHT) * height,
          left: (806 / DESIGN_WIDTH) * width,
          width: (246 / DESIGN_WIDTH) * width,
          height: (102 / DESIGN_HEIGHT) * height,
        }]}
        onPress={handleMovePress}
        activeOpacity={0.7}
      />
